...
export const seed = async function (knex): Promise<any> {
    ...
    for (const item of data) {
        if (item.meta.tags) {
            // @ts-ignore
            item.tags = await Tag.query(knex).whereIn('name', item.meta.tags)
        }
        if (item.meta.internalTags) {
            // @ts-ignore
            item.internalTags = await Tag.query(knex).whereIn('name', item.meta.internalTags)
        }
    }

    ...
    <CuratorPanel>
      <CuratorInfo title>
        ...
        >
          ...
          <Button to="/curator/application" class="outline-white">Become a curator</Button>
        </Claim>
        ...
        <Button status="underline" @click="showClaimPopup">Created this game?</Button>
      </CuratorInfo>
    </CuratorPanel>
  </div>
</template>
...
export default {
  components: {
    ...
    CuratorPanel: () =>
      import("@ericmuyser/hyper-ui").then(m => m.CuratorPanel),
    CuratorInfo: () => import("@ericmuyser/hyper-ui").then(m => m.CuratorInfo),
    JoinCommunity: () =>
      import("@ericmuyser/hyper-ui").then(m => m.JoinCommunity),
  ...
  data() {
    return {
      links: {
        gameOverview: [
          {
            to: { path: "/help/topic/1/article/suggesting-features" },
            title: "Suggesting Features"
          },
          {
            to: { path: "/help/topic/1/article/voting-and-curating-updates" },

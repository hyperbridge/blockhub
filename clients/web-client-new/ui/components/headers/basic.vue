    ...
    <header class="app-header">
        ...
        <div class="position-relative w-100" style="margin-top: -10px; zoom: 0.9;">
            ...
            <div class="app-header__options" v-if="signedIn && developerMode">
                <button class="remove-btn btn btn-secondary btn-block btn--icon btn--icon-left" @click="clickRemove()" v-if="!isViewing">
                ...
                <button class="publish-btn btn btn-secondary btn-block btn--icon btn--icon-left" @click="clickPublish()" v-if="!isViewing">
                ...
                <button class="exit-btn btn btn-secondary btn-block btn--icon btn--icon-left" @click="clickExit()" v-if="!isViewing">
                    <p><span class="fa fa-arrow-alt-circle-left"></span> Back</p>
                </button>
            </div>
            <nav class="app-header__nav hide-on-mobile" v-if="!signedIn || (signedIn && isViewing)">
    ...
    computed: {
        ...
        currentLanguage() {
            ...
            if (!this.account.language || !this.account.language.code)
                return this.languages.find((el) => !!(navigator.language || navigator.userLanguage).toLowerCase().includes(el.code.toLowerCase()))

            // If that failed, set to default: US
            if (!this.account.language || !this.account.language.code)
                return this.languages.find((el) => !!el.code.toLowerCase().includes('us'))
